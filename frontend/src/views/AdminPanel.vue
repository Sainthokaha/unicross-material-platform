                  ...
                  <td class="px-6 py-4">
                    <div class="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                      ...
                      <template v-if="m.status === 'pending'">
                        <button @click="materialsStore.approveMaterial(m.id)" class="bg-green-100 text-green-700 hover:bg-green-200 px-3 py-1.5 rounded text-xs font-bold transition flex items-center gap-1">
                          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                          Approve
                        </button>
                        <button @click="handleReject(m.id)" class="bg-red-100 text-red-700 hover:bg-red-200 px-3 py-1.5 rounded text-xs font-bold transition flex items-center gap-1">
                      ...
                      <template v-else>
                        <span class="text-red-600 text-xs font-bold flex items-center gap-1 bg-red-50 px-2 py-1 rounded">
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                          Rejected
                        </span>
                        <span v-if="m.rejection_reason" :title="m.rejection_reason" class="text-gray-500 text-xs truncate max-w-[120px] cursor-help border-b border-dotted border-gray-400">
                          ({{ m.rejection_reason }})
                        </span>
                      </template>
                    </div>
          ...
          <form @submit.prevent="handleCreateUser" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-if="userForm.role === 'student'">
                ...
                <input v-model="userForm.matric_number" type="text" class="form-input" placeholder="e.g., 001" />
              </div>
              <div v-if="userForm.role !== 'student'">
