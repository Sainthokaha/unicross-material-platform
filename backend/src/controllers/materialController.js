...
const getMaterials = async (req, res) => {
  try {
    ...
    const params = [];
    const conditions = [];
    if (req.user.role === 'lecturer') { conditions.push('m.uploaded_by = ?'); params.push(req.user.id); } 
    else if (req.user.role === 'student') { 
      conditions.push("m.status = 'approved'"); 
      if (req.user.department_id) { conditions.push('c.department_id = ?'); params.push(req.user.department_id); } 
      else { conditions.push('c.department_id = -1'); } 
    }
