...
const forgotPassword = async (req, res) => {
    try {
        ...
        await resend.emails.send({
            ...
            html: `<p>Click to reset: <a href="${resetUrl}">Link</a></p>`
        });
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const [users] = await db.query('SELECT * FROM users WHERE verification_token = ? AND verification_token_expiry > ?', [verificationTokenHash, new Date()]);
        if (users.length === 0) return res.status(400).json({ success: false, message: 'Invalid or expired verification token' });
        
        const user = users[0];
        await db.query('UPDATE users SET email_verified = 1, verification_token = NULL, verification_token_expiry = NULL WHERE id = ?', [user.id]);
